...
**Onboarding**
- Entry contract: any navigation into a shell screen (Movements, Stats, Settings — including via the wildcard redirect or the landing's "Open the app" CTA) by a user without a completed onboarding is redirected to Onboarding. Public pages (Landing, Privacy) are exempt and never trigger onboarding. Enforcement lives in the router as a navigation guard on the shell routes, not in a boot-time check, so every entry point (deep link, landing CTA, unknown URL) is covered.
- On entering Onboarding, check for a local profile record (language, base currency, onboardingCompleted flag). If absent, show it. The first step asks the preferred Language (preselected from the browser language); the second offers an optional restore (restore from cloud or upload a backup file, or start fresh — restoring applies the backup's language over the selected one); when declined, proceed to set base currency, create initial accounts, review/edit pre-populated default categories. No pay-day rule configuration (periods are manually selected month names).
