...
**The Negative-Stays-Ink Rule.** A sign never flips a color: negative figures render in On Surface ink, whatever the page. Charts keep ink at every sign — the Stats year overview's Net columns and the balance strip's overdrawn months stay ink; inside a chart a sign is a shape the reader compares against the zero line, not a warning (ADR 0021). Derived figures — the Movements Net Flow net, the Stats Net and Avg Monthly Savings, the year-spine months — are information, not money moving, so they stay ink at every sign; a minus is a fact, not an alarm. The one exception is the balance list: an overdrawn account balance renders in Error red there, because it warns about money owed rather than plotting a data point. Red marks movement direction (expense figures and expense stripes) and warnings only (error strips, destructive confirmations, that overdrawn balance); green marks money in; grey carries no direction (ADR 0011).
...
Cards stack vertically with 1.5rem (24px) gutter — the `--space-lg` section rhythm every stacked card and section shares (ADR 0019 folded the Stats KPI trio into one card that rides this same rhythm). Forms lay out on a two-column grid; the Quick Add card's compact mode gives the amount column the widest track. Tables are full-width, `border-collapse: collapse`, with 0.5rem cell padding, 1px tonal row rules, and a `--row-hover` (#f5f5f5) hover.
...
- **Danger:** two sanctioned treatments — **filled** Error with white text for panel/card-level destructive confirmations (restore), and **outline** Error text and border for the destructive *confirmation* moment (the tick/X pair) in a row or dense list. The delete *affordance* (trash) is never red: red on every row would dilute the red that marks money out — Error red on a ledger page is expense figures, stripes, and destructive intent, never an idle offer (the Ledger Green Rule's mirror: red warns, it doesn't lurk). The one exception is the mass/irreversible action: a control that destroys everything at once (onboarding's Delete all) may wear outline Error at rest, because red = mass/irreversible — it is the red that warns, not a red that lurks; single-row affordances stay neutral.
...
- **Icon-only:** row actions that need no text render as small square icon buttons (pencil Edit, trash Delete, tick/X delete confirm/cancel) — inline SVG at `1rem`, `currentColor` stroke, no visible label; the accessible name comes from `aria-label` mirrored to `title`. Confirm/cancel swaps the whole action pair and announces the deleted-movement prompt through a visually-hidden polite live region, never visible message text.
- **Touch (ghost row actions):** on coarse-pointer viewports (and below 480px) every primary control rises to the 44px thumb minimum — the icon-only row actions as 44px squares, and the text-bearing capture triggers (New Transaction / New Transfer) and capture-form Cancel/Save as a 44px `min-height`. Desktop keeps its compact sizing; the press interaction (shadow shed on `:active`) is untouched by the resize. At those same viewports the icon-only row actions render as **ghosts**: the box comes off (no border, no press shadow, flat surface fill) and the glyph speaks alone — muted On Surface Variant ink for Edit, Delete, and cancel; the Error red glyph appears only on the confirm tick, at the moment of destructive intent. One row-action grammar shared by the movement ledger and the Settings card rows (pencil/trash/tick/X, mirrored exactly); the edit-state primary tick keeps its Primary box — it is the form's action, not a row utility. A row utility whispers; the boxed press chrome is reserved for primary actions.
- **Disabled:** 0.6 opacity, default cursor, no press.

### Cards / Containers
- **Card:** Surface Lowest, 1px hard black border, 1rem padding. The universal unit of Stats and Settings content. Card headers separate from the body by spacing alone — no internal divider rule (dropped deliberately: internal rules are reserved for data structure, i.e. table rows). The one exception is the Stats KPI Ledger Summary, whose own padding drops to 0 so its direction stripes touch the card edge (see Signature Components, ADR 0019).
...
- **Backup Banner:** a non-interactive full-width status strip (Primary Fixed fill, Primary bottom hairline) with its content aligned to the centered content column via the chrome gutter. Status reads left: the method label ("Google Drive") in Primary Deep at 600, with the relative-time caption beneath it; a compact primary **Back up** button (`%btn-primary`/`%btn-small`) sits pinned right and is the strip's only interactive element. Offline, it degrades to Surface Container Low with the method and caption in On Surface and renders a genuinely disabled neutral button. Backup failures render an error strip in the error-container family below the banner, announced via `role="alert"`, with a small outline-danger Dismiss button. Its sibling Install Prompt uses the same strip language. The chrome strips never stack: while the Update Prompt is visible the Install Prompt yields its slot and returns only after the update banner is resolved — the optional suggestion never competes with the urgent notice.
- **Movement Rows:** no Type column and no direction arrows — the 4px left edge stripe is the type's only signal: green (income), red (expense), grey (transfer). Amounts always render positive in JetBrains Mono, colored only by direction: Income green, Error red; transfers stay neutral ink. A Transaction on a Credit Card (a Card Purchase or a card refund) keeps its direction stripe — red for a purchase, green for a refund — and wears an outline **Card** badge beside its category chip: the stripe carries money direction, the badge names the account kind, because card spending is credit and stays out of the cash-basis Expenses until the Card Payment (ADR 0023). A Card Payment (a Transfer into a credit card) shows its payment category as a chip like a Transaction, with the `source → card` route as a muted line beneath it. Actions are icon-only: pencil Edit and trash Delete in the same neutral ink (an idle delete is not a warning — see Buttons › Danger); Delete swaps the pair for tick (confirm, outline danger — the one red action) / X (cancel) icon buttons with the prompt announced off-screen, and the Undo Toast covers recovery. On mobile (≤ 480px) the rows group under caps-label day dividers; see Layout for the day-section card spec.
- **KPI Ledger Summary (ADR 0019):** Stats' year-to-period figures read as one ledger block: a single card whose caps scope header (`2026 · JAN–SEP`) sits over three full-bleed rows separated by 1px Surface Container Low rules — the table-row grammar, since these are data rows — each carrying the 4px direction stripe: green Income, red Expenses, grey Net (the Net-Flow stripe; a derived Net is information, not money moving). Every row pairs a caps metric label on the left with a right-aligned figure block: the year total at headline size in JetBrains Mono, its `AVG …/MO` monthly average demoted beneath in mono caption ink — three totals aligning into a single tabular ledger column. The Savings Rate renders as a footer line (caps label left, mono figure right) only when income makes it meaningful. A figure outgrowing its line drops whole to its own full-width right-aligned line (`flex-wrap`), never breaking mid-number; the `FitTextDirective` shrink-to-fit stays on as a silent safety net. The prior trio's solid-ink inverted Net card retired with it: Net is ink at every sign (the Negative-Stays-Ink Rule), carried by position and the grey stripe.
